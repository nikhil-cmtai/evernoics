  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        style={{
          background: 'linear-gradient(180deg, #f8fafc 60%, #fffbe6 100%)'
        }}
      >
        <motion.h2
          variants={cardVariants}
          className="text-4xl font-extrabold mb-2 mt-2"
          style={{
            color: '#f97316',
            background: 'linear-gradient(90deg, #f97316 60%, #facc15 100%)',
      ...
      {/* 5. Industries We Serve */}
      <motion.div variants={containerVariants} className="mb-14 w-full">
        <motion.h2 variants={cardVariants} className="text-2xl font-bold text-[var(--color-primary)] mb-6">
          Industries We Serve
        </motion.h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-7">
          {industries.map((item) => ( // <-- removed ", idx"
            <motion.div
              ...
              className="bg-white/80 rounded-2xl shadow-lg p-6 text-left flex flex-col gap-2 transition-all"
              style={{ borderTop: '3px solid var(--color-secondary)' }}
            >
              <div className="flex items-center gap-3 mb-1">{item.icon}<span className="font-semibold text-[var(--color-primary)]">{item.label}</span></div>
              <div className="text-gray-700 text-sm">{item.desc}</div>
            </motion.div>
          ))}
        </div>
